...
The mean is **`r meanTSPD`** and median is **`r medianTSPD`** for total number of steps taken per day.
...
**`r names(maxStepsInterval)`** is the 5-minute interval which contains the maximum number of steps **`r maxStepsInterval`**, on average across all the days.
...
There are **`r numberOfRowsWithNA`** in the dataset with missing values.
...
After imputing NA values with mean of its respective interval, the mean is **`r meanTSPDImputed`** and median is **`r medianTSPDImputed`** for total number of steps taken per day.
